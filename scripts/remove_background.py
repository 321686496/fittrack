...
def remove_background(img, bg_color, tolerance, feather):
    """去除纯色背景，返回透明 PNG 图像（使用超采样抗锯齿）"""
    original_size = img.size
    scale = 2
    img_upscaled = img.resize((original_size[0] * scale, original_size[1] * scale), Image.LANCZOS)
    img_upscaled = img_upscaled.convert("RGBA")
    rgb = np.asarray(img_upscaled.convert("RGB"), dtype=np.float32)
    ...
    result_upscaled = Image.fromarray(rgba, "RGBA")
    result = result_upscaled.resize(original_size, Image.LANCZOS)
    return result
